...
function Home(){
    var titulo = '​Bienvenidos a DH Movies el mejor sitio para encontrar las mejores  películas, incluso mucho mejor que Netflix, Cuevana y PopCorn​.\n';

    var totalPeli = 'Peliculas en cartelera ' + totalpeliculas + '\n\n';
    const listadoPeli = 'Listado de películas:'
    
    const peliculas = movies.map(movie => movie.title);
    peliculas.sort();
	
	let peliLista = ' \n';
    for (let unaPeli of peliculas) {
        peliLista += unaPeli + '\n';
    }
    var piePagina =  `\nRecordá que podés visitar las secciones:\n •En Cartelera (/en-cartelera)\n •Más Votadas (/mas-votadas)\n •Sucursales (/sucursales)\n •Contacto (/contacto)\n •Preguntas Frecuentes (/preguntas-frecuentes)`;
    return titulo + totalPeli + listadoPeli + peliLista + piePagina;
...
http.createServer((req, res) => {
	...
	switch (req.url) {
		...
		/*
			...
			c. Listados de películas. ​Para cada película mostrar:
			i. Título (​ título de la película)
			ii. Reseña​ (reseña de la película)
		*/
		case '/en-cartelera':
			//a. Título​: ​En Cartelera.
			let cartelera  = `\n En cartelera`;
			
			//b. Total de películas​: ​(total de películas)
			cartelera += `\n\n\n Total de películas en cartelera: ${movies.length}`;
			
			/*c. Listados de películas. ​Para cada película mostrar:
			i. Título (​ título de la película) | 	ii. Reseña​ (reseña de la película)
			*/
			cartelera += `\n Listados de Películas: \n\n `;
			
			for  (let movie of movies){
				cartelera += `\t\t${movie.title} \n\n ${movie.overview} \n\n`;
			}
			res.end(cartelera);
			break;
		...
		case '/mas-votadas':
			...
			let valores = masVotadas.map(movie => movie.vote_average);
            let redu = valores.reduce((previo, actual) => actual += previo);
            infoMasVotadas += `\n\n\n Rating promedio: ${(redu / valores.length).toFixed(2)}`;
            console.log(infoMasVotadas);
			...
			res.end(infoMasVotadas);
			break;
		
		case '/sucursales':
			/*
			a. Título​: ​Nuestras Salas.
			b. Total de salas:​ ​(total de salas)
			c. Listados de salas. ​Para cada sala mostrar:
			i. Nombre ​(nombre de la sala)
			ii. Dirección​ (ubicación de la sala)
			iii. Descripción​ (descripción de la sala)
			*/
			let sucursales = `\n\n Nuestras Salas \n\n`;
		...
		case '/contacto':
			/*
			a. Título​: ​Contáctanos.
			b. Contenido​: ¿​ Tenés algo para contarnos? Nos encanta escuchar a nuestros
			clientes. Si deseas contactarnos podés escribirnos al siguiente email: dhmovies@digitalhouse.com o en las redes sociales. Envianos tu consulta, sugerencia o reclamo y será respondido a la brevedad posible. Recordá que también podes consultar la sección de Preguntas Frecuentes para obtener respuestas inmediatas a los problemas más comunes.
			*/
			let contacto = `\nContactános\n` ;
			contacto+= `¿​ Tenés algo para contarnos?\n\nNos encanta escuchar a nuestros clientes. \n Si deseas contactarnos podés escribirnos al siguiente email: dhmovies@digitalhouse.com o en las redes sociales. Envianos tu consulta, sugerencia o reclamo y será respondido a la brevedad posible. Recordá que también podes consultar la sección de Preguntas Frecuentes para obtener respuestas inmediatas a los problemas más comunes.`;
			res.end(contacto);
			break;
		
		case '/preguntas-frecuentes':
			/*
			a. Título​: ​Preguntas Frecuentes.
			b. Total de preguntas​: ​(total de preguntas)
			c. Listados de preguntas. ​Para cada preguntas mostrar:
			i. Pregunta ​(título de la pregunta)
